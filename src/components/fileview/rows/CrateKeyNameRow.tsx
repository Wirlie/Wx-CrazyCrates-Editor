...
function CrateKeyNameRow(props: Props) {
    ...
    React.useEffect(() => {
        setUnsavedValue(value ?? t("crate_editor_key_name_default"))
    }, [value, t])
    ...
    if(!enableEdit) {
        return (
            <tr>
                <td style={{verticalAlign: "middle", whiteSpace: "nowrap"}}><FontAwesomeIcon icon={faTag} /> <b>{t("crate_editor_key_name_title")}</b></td>
        ...
        return (
            <tr>
                <td style={{verticalAlign: "middle", whiteSpace: "nowrap"}}><FontAwesomeIcon icon={faTag} /> <b>{t("crate_editor_key_name_title")}</b></td>
                <td className="w-100"><input type="text" value={unsavedValue} className="form-control" onChange={(e) => setUnsavedValue(e.target.value)} placeholder={t("crate_editor_key_name_input_placeholder")} /></td>
...
export default CrateKeyNameRow
