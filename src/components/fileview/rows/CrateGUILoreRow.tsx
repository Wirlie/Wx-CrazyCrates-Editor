...
function CrateGUILoreRow(props: Props) {
    ...
    if(!enableEdit) {
        return (
            <tr>
                <td style={{verticalAlign: "middle", whiteSpace: "nowrap"}}><FontAwesomeIcon icon={faAlignLeft} /> <b>{t("crate_editor_lore_title")}</b></td>
        ...
        return (
            <tr>
                <td style={{verticalAlign: "middle", whiteSpace: "nowrap"}}><FontAwesomeIcon icon={faAlignLeft} /> <b>{t("crate_editor_lore_title")}</b></td>
                <td className="w-100">
                    <textarea placeholder={t("crate_editor_lore_input_placeholder")} rows={10} value={unsavedValue.join("\n")} className="form-control" onChange={(e) => setUnsavedValue(e.target.value.split("\n"))} />
                </td>
...
export default CrateGUILoreRow
